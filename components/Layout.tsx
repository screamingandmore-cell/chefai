...
export const AdBanner: React.FC<{ type: 'banner' | 'interstitial' }> = ({ type }) => {
  if (type === 'banner') {
    return (
      <div className="w-full mx-auto max-w-[90%] mt-4 h-[90px] bg-gray-100 rounded-xl flex flex-col items-center justify-center text-gray-400 text-xs text-center border border-dashed border-gray-300">
...
export const Layout: React.FC<LayoutProps> = ({ children, activeView, onNavigate, isPremium }) => {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-[#F5F7FA] sm:max-w-md sm:mx-auto sm:border-x sm:border-gray-200 shadow-2xl relative font-sans text-gray-800">
      ...
      {/* Header Minimalista */}
      <header className="bg-white px-6 pt-12 pb-4 flex justify-between items-center sticky top-0 z-30 shadow-[0_2px_15px_-3px_rgba(0,0,0,0.03)]">
      ...
      <nav className="fixed bottom-6 left-4 right-4 bg-white/90 backdrop-blur-md rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] border border-white/50 flex justify-between items-center py-3 px-6 z-40 max-w-[calc(28rem-2rem)] sm:mx-auto">
        {navItems.map((item) => {
