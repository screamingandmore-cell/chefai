      ...
      <header className="bg-white p-4 sticky top-0 z-20 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => onNavigate(ViewState.HOME)}>
          <img 
            src="/icon-192.png" 
            alt="Logo Chef.ai" 
            className="w-8 h-8 rounded-lg object-contain bg-gray-900" 
            onError={(e) => {
              // Fallback para o ícone SVG (Chapéu) se o PNG não existir
              e.currentTarget.src = '/favicon.svg';
              e.currentTarget.style.backgroundColor = 'transparent';
              // Remove o handler para evitar loop infinito se o SVG tbm falhar
