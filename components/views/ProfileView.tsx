...
export const ProfileView: React.FC<ProfileViewProps> = ({ user, session, onLogout, onDeleteAccount, onNavigate }) => {
  return (
    <div className="flex flex-col min-h-full pb-32 animate-slideUp px-4">
       ...
       <div className={`p-6 rounded-[2.5rem] border shadow-soft flex items-center justify-between transition-all mb-8 ${
       ...
       <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-soft overflow-hidden mb-12">
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <span className="font-bold text-sm text-gray-700 group-hover:text-red-500 transition-colors">Sair da Conta</span>
            </div>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-4 h-4 opacity-30">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
            </svg>
       ...
       {/* Rodapé Legal e Zona de Perigo */}
       <div className="mt-auto pt-8 border-t border-gray-50 text-center space-y-6">
          <button 
            onClick={() => onNavigate(ViewState.TERMS)}
            className="text-[11px] text-gray-400 font-bold uppercase tracking-widest hover:text-gray-600 transition-colors"
          >
          ...
          <div className="pb-4">
            <button 
              onClick={() => {
                if (confirm("Tem certeza que deseja excluir sua conta permanentemente? Esta ação é irreversível.")) {
                  onDeleteAccount();
                }
              }} 
              className="text-red-400 hover:text-red-600 font-black text-[10px] uppercase tracking-[0.2em] transition-colors"
            >
              Excluir Minha Conta
            </button>
