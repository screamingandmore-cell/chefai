...
export const saveWeeklyMenu = async (userId: string, menu: WeeklyMenu): Promise<WeeklyMenu> => {
  ...
  // We omit 'id' from the top level insert to let Supabase generate it.
  // We keep the structure for the JSON 'data' field, but we will sync the ID after insert.
  ...
  // Return the menu with the ID generated by the database
  return {
    ...data.data,
    id: data.id,
